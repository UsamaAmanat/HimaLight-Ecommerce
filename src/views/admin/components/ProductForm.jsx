...
import { uploadToS3 } from "../../../helpers/s3Upload.js";
...
const ProductForm = ({ product, onSubmit, isLoading }) => {
  ...
  const onSubmitForm = async (form) => {
    ...
    // --- UPLOAD THUMBNAIL ---
    let thumbnailUrl = product.imageUrl;
    if (imageFile.image.file) {
      thumbnailUrl = await uploadToS3(imageFile.image.file, "thumbnails");
    }

    // --- UPLOAD IMAGE COLLECTION ---
    const collectionUrls = [];
    if (imageFile.imageCollection.length > 0) {
      for (const img of imageFile.imageCollection) {
        if (img.file) {
          const url = await uploadToS3(img.file, "gallery");
          collectionUrls.push(url);
        } else if (img.url) {
          // Keep old existing image if editing
          collectionUrls.push(img.url);
        }
      }
    }
    ...
    onSubmit({
      ...
      dateAdded: Date.now(),
      image: thumbnailUrl,
      imageCollection: collectionUrls,
    });
  };
